...
struct MaterialInputs
{
    ...
    float   Roughness;                  // Defines the perceived smoothness (0.0) or roughness (1.0). It is sometimes called glossiness.
#endif
...
void SpecialEmissiveLight( const ShadingParams shading, const LightParams light, inout float3 diffuseColor ) 
{
    ...
    if( light.AddToEmissive )
        diffuseColor    += (g_Global.GlobalSpecialEmissiveScale * light.ColorIntensity.w) * light.ColorIntensity.rgb * shading.PrecomputedEmissive;
#endif
}
...
float normalFiltering(float perceptualRoughness, const vec3 worldNormal) {
    ...
    vec3 du = ddx( worldNormal );
    vec3 dv = ddy( worldNormal );
 ...
 * testing fails.
 */
PixelParams ComputePixelParams(const ShadingParams shading, const MaterialInputs material) // was getPixelParams
{

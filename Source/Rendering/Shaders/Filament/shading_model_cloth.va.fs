...
void surfaceShading( const ShadingParams shading, const PixelParams pixel, const LightParams light, float occlusion, inout float3 inoutDiffuseColor, inout float3 inoutSpecularColor )
{
    ...
    inoutDiffuseColor    += Fd * lightColor;
    inoutSpecularColor   += (Fr * NoL) * lightColor;     // We need to apply NoL separately to the specular lobe since we already took it into account in the diffuse lobe
#else
    // vec3 color = Fd + Fr;
    ...
    inoutDiffuseColor    += Fd * lightColor;
    inoutSpecularColor   += Fr * lightColor;
#endif
}

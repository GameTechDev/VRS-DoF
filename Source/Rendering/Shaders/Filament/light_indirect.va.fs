...
vec3 DiffuseIrradiance( const ShadingParams shading, vec3 n ) 
{
    ...
    [branch] if( shading.IBL.UseLocal )
    ...
    [branch] if( shading.IBL.UseDistant )
...
vec3 prefilteredRadiance( const ShadingParams shading, vec3 r, float perceptualRoughness ) 
{
...
void evaluateClearCoatIBL( const ShadingParams shading, const PixelParams pixel, float specularAO, inout vec3 Fd, inout vec3 Fr ) {
#if IBL_INTEGRATION_ALGORITHM == IBL_INTEGRATION_IMPORTANCE_SAMPLING
    ...
    Fd *= attenuation;
    Fr *= attenuation;
    Fr += prefilteredRadiance( shading, clearCoatR, pixel.clearCoatPerceptualRoughness ) * (specularAO * Fc);
#endif
}

void evaluateSubsurfaceIBL( const ShadingParams shading, const PixelParams pixel, const vec3 diffuseIrradiance, inout vec3 Fd, inout vec3 Fr )
{
#if defined(SHADING_MODEL_SUBSURFACE)
    vec3 viewIndependent = diffuseIrradiance;
    vec3 viewDependent = prefilteredRadiance( shading, -shading.View, pixel.Roughness, 1.0 + pixel.Thickness );
...
#error "invalid REFRACTION_TYPE"
    ...
    vec3 Ft = prefilteredRadiance( shading, ray.direction, perceptualRoughness ) * frameUniforms.iblLuminance;
#else
...
void evaluateIBL( const ShadingParams shading, const MaterialInputs material, const PixelParams pixel, inout float3 diffuseColor, inout float3 specularColor )
{
    // Apply transform here if we wanted to rotate the IBL
    vec3 n = shading.Normal;

    float ssao = evaluateSSAO();
    float diffuseAO = shading.CombinedAmbientOcclusion;// min( material.AmbientOcclusion, ssao );
    ...
    Fr = E * prefilteredRadiance( shading, r, pixel.PerceptualRoughness );
#elif IBL_INTEGRATION_ALGORITHM == IBL_INTEGRATION_IMPORTANCE_SAMPLING
    vec3 E = vec3(0.0.xxx); // TODO: fix for importance sampling
    Fr = isEvaluateIBL( pixel, shading.Normal, shading.View, shading.NoV );
#endif
    ...
    vec3 diffuseIrradiance = DiffuseIrradiance( shading, n );
    ...
    combineDiffuseAndSpecular(pixel, n, E, Fd, Fr, diffuseColor, specularColor);
}

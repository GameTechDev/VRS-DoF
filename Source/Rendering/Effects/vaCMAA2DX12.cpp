...
#error Forgot to include CMAA2.hlsl?
    ...
    // Update shaders to match input/output format permutations based on support
    {
        ...
        m_CSEdgesColor2x2->CreateShaderFromFile( L"vaCMAA2.hlsl", "cs_5_0", "EdgesColor2x2CS", shaderMacros, false );
        m_CSProcessCandidates->CreateShaderFromFile( L"vaCMAA2.hlsl", "cs_5_0", "ProcessCandidatesCS", shaderMacros, false );
        m_CSDeferredColorApply2x2->CreateShaderFromFile( L"vaCMAA2.hlsl", "cs_5_0", "DeferredColorApply2x2CS", shaderMacros, false );
        m_CSComputeDispatchArgs->CreateShaderFromFile( L"vaCMAA2.hlsl", "cs_5_0", "ComputeDispatchArgsCS", shaderMacros, false );
        m_CSDebugDrawEdges->CreateShaderFromFile( L"vaCMAA2.hlsl", "cs_5_0", "DebugDrawEdgesCS", shaderMacros, false );
...
vaDrawResultFlags vaCMAA2DX12::Execute( vaRenderDeviceContext & deviceContext, ID3D12GraphicsCommandList* commandList, const InputResourceHelperDX12 & inOutColor, const InputResourceHelperDX12 & optionalInLuma, const InputResourceHelperDX12 & inColorMS, const InputResourceHelperDX12 & inColorMSComplexityMask )
{
    ...
    {
        m_firstRun = false;
        VA_TRACE_CPUGPU_SCOPE( ClearControlBuffer, deviceContext );
    ...
    {
        VA_TRACE_CPUGPU_SCOPE( DetectEdges2x2, deviceContext );
    ...
    {
        VA_TRACE_CPUGPU_SCOPE( ComputeDispatchArgs1CS, deviceContext );
        commandList->SetPipelineState( m_PSOComputeDispatchArgsPass.Get() );
        commandList->Dispatch( 2, 1, 1 );
    ...
    {
        VA_TRACE_CPUGPU_SCOPE( ProcessCandidates, deviceContext );
    ...
    {
        VA_TRACE_CPUGPU_SCOPE( ComputeDispatchArgs2CS, deviceContext );
        commandList->SetPipelineState( m_PSOComputeDispatchArgsPass.Get() );
    ...
    {
        VA_TRACE_CPUGPU_SCOPE( DeferredColorApply, deviceContext );
    ...
    if( m_debugShowEdges )
    {
        VA_TRACE_CPUGPU_SCOPE( DebugDrawEdges, deviceContext );
...
vaDrawResultFlags vaCMAA2DX12::DrawMS( vaRenderDeviceContext & deviceContext, const shared_ptr<vaTexture> & inoutColor, const shared_ptr<vaTexture> & inColorMS, const shared_ptr<vaTexture> & inColorMSComplexityMask )
{
    ...
    AsDX12( deviceContext ).CommitRenderTargetsDepthStencilUAVs( );

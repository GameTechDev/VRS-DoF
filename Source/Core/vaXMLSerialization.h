...
namespace Vanilla
{
    ...
    // SFINAE - https://en.cppreference.com/w/cpp/language/sfinae / https://stackoverflow.com/questions/257288/is-it-possible-to-write-a-template-to-check-for-a-functions-existence / https://stackoverflow.com/questions/87372/check-if-a-class-has-a-member-function-of-a-given-signature/10707822
    template <typename T>
    class has_static_serialize
    {
        typedef char YesType[1];
        typedef char NoType[2];
        template <typename C> static YesType& test( decltype( &C::StaticSerialize ) );
        template <typename C> static NoType& test( ... );

    public:
        //enum { value = sizeof(test<T>(0)) == sizeof(YesType) };
        static const bool has = sizeof(test<T>(0)) == sizeof(YesType);
    };
    ...
    private:
        ...
        map<string, vaSerializableObjectConstructor>
    ...
    private:
        ...
        template< typename ValueType >
        bool                        SerializeInternal( const char* name, ValueType & val, std::true_type )                  { if( !SerializeOpenChildElement( name ) ) return false; bool retVal = ValueType::StaticSerialize( *this, val ); if( !SerializePopToParentElement( name ) ) { assert( false ); return false; }; return retVal; }
    ...
    public:
        ...
        template< typename ValueType >
        bool                        Serialize( const char * name, ValueType & val )                                         { return SerializeInternal< ValueType >( name, val, std::integral_constant<bool, has_static_serialize<ValueType>::has>() ); }
        //
        ...
        bool                        SerializeArray( const char * containerName, vector< ItemType > & elements );

        template< typename ItemType >
        bool                        SerializeArray( const char * containerName, vector< shared_ptr<ItemType> > & elements );

        template< typename vaXMLSerializableObjectType >
        bool                        TypedSerializeArray( const char * containerName, vector< shared_ptr<vaXMLSerializableObjectType> > & elements );
    ...
    template< typename ItemType >
    inline bool vaXMLSerializer::SerializeArray( const char * containerName, vector< ItemType > & elements )  
    { 
        assert( m_formatVersion >= 0 );
        return SerializeArrayGeneric<vector<ItemType>>( containerName, elements, 
    ...
    template< typename ItemType >
    inline bool vaXMLSerializer::SerializeArray( const char * containerName, vector< shared_ptr<ItemType> > & elements )  
    {
        assert( m_formatVersion >= 0 );
        return SerializeArrayGeneric<vector<shared_ptr<ItemType>>>( containerName, elements, 
    ...
    template< typename vaXMLSerializableObjectType >
    inline bool vaXMLSerializer::TypedSerializeArray( const char * containerName, vector< shared_ptr<vaXMLSerializableObjectType> >& elements )
    {
        assert( m_formatVersion >= 0 );
        return SerializeArrayGeneric<vector<shared_ptr<vaXMLSerializableObjectType>>>( containerName, elements,
            [ ]( bool isReading, vector<shared_ptr<vaXMLSerializableObjectType>>& container, int& itemCount )
        {
            if( isReading )
            {
                container.resize( itemCount );
            ...
            [ ]( vaXMLSerializer& serializer, vector<shared_ptr<vaXMLSerializableObjectType>>& container, int index )
        {
